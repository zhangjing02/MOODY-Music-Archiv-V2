...
func getCoreName(name string) string {
	...
	name = strings.Split(name, "（")[0]
	// 简繁映射表 (覆盖审计出的重点及常用艺人 - V12.39 全量版)
...
func main() {
	fmt.Println("=== MOODY V12.39 核弹级物理去重程序 ===")
	...
	if err == nil {
		...
		fmt.Println("[JSON] 物理全量回归完成。")
	}

	fmt.Println("=== V12.39 物理治理圆满结束 ===")
}
